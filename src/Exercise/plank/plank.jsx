...
export default function PlankTimerOptimized() {
  ...
  // ✅ Mediapipe safety refs (fix for "instance already deleted")
  ...
  // ✅ helper: stop everything safely (no double-close)
  const stopPoseAndCamera = () => {
  ...
  useEffect(() => {
    if (alreadyDone) {
      // if already completed, ensure camera/pose stopped
      stopPoseAndCamera();
      return;
    }
      ...
      const sendFrame = async () => {
        ...
        if (
          ...
          try {
            ...
            // if pose already closed during a frame, just stop quietly
            runningRef.current = false;
            return;
          }
        }
      ...
      if (results.poseLandmarks) {
        ...
        if (goodPlank && !success) {
          ...
          if (elapsed >= targetSeconds) {
            ...
            // ✅ STOP LOOP FIRST, THEN CLOSE ONCE
            stopPoseAndCamera();
          }
        } else {
  ...
  }, [alreadyDone, targetSeconds, maxPoints, success]);
