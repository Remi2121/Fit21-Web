...
export default function PushUpCounter() {
  ...
  useEffect(() => {
    ...
    async function loadDailyPoints() {
      try {
        const today = new Date().toISOString().split("T")[0];
        const ref = doc(db, "users", userId, "exercises", exerciseId); // lowercase "exercises"
        const snap = await getDoc(ref);
        ...
        // Same day → load existing
        if (!cancelled) setCount(typeof data.points === "number" ? data.points : 0);
        if (!cancelled) setLoaded(true);
      } catch (err) {
        console.error("Error loading daily points:", err);
        if (!cancelled) setLoaded(true); // allow saves after error so app still works
      }
    }
  ...
  useEffect(() => {
    ...
    async function savePoints() {
      try {
        const today = new Date().toISOString().split("T")[0];
        // <-- FIX: use lowercase "exercises" so save and load use the same path
        const ref = doc(db, "users", userId, "exercises", exerciseId);
